...
[Riferimento endpoint e-mail](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails) Per la manipolazione delle risorse e-mail viene fornito un set completo di endpoint REST.

Nota: se utilizzi [Contenuto predittivo Marketo](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/predictive-content/working-with-predictive-content/understanding-predictive-content), i seguenti endpoint non avranno esito positivo se fanno riferimento a un’e-mail con contenuto predittivo: [Ottieni contenuto e-mail](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/getEmailContentByIdUsingGET), [Sezione Aggiorna contenuto e-mail](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/updateEmailComponentContentUsingPOST), [Approva bozza e-mail](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/approveDraftUsingPOST). La chiamata restituisce un codice di errore 709 e il messaggio di errore corrispondente.

## Query

Il modello di query per le e-mail è identico a quello dei modelli, consentendo le query [per ID](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/getEmailByIdUsingGET), [per nome](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/getEmailByNameUsingGET), e [navigazione](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/getEmailUsingGET), e per filtrare in base alla cartella con le API Sfoglia e per nome.
...
La navigazione nelle cartelle funziona come altri endpoint di navigazione dell’API delle risorse e consente di filtrare facoltativamente su `status`, `folder`, `earliestUpdatedAt`/`latestUpdatedAt`, `maxReturn`, e `offset`. `status` è Approvato o Bozza. `folder` è un oggetto JSON contenente `id` e `type`. `maxReturn` è un numero intero che limita il numero di risultati (il valore predefinito è 20, il valore massimo è 200) e `offset` è un numero intero che può essere utilizzato con `maxReturn` per leggere i set di risultati di grandi dimensioni (il valore predefinito è 0).
...
Le sezioni possono restituire un tipo di dynamicContent. Consulta la [Contenuto dinamico](dynamic-content.md) per ulteriori informazioni.

## Interroga campi CC

Puoi recuperare il set di campi abilitati per E-mail CC nell’istanza di destinazione chiamando il [Ottieni campi CC e-mail](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/getEmailCCFieldsUsingGET) endpoint.
...
[Le e-mail vengono create](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/createEmailUsingPOST) basato su un modello di origine e dispongono di un elenco di sezioni modificabili derivate da ogni singolo elemento HTML in tale modello con una classe &quot;mktEditable&quot; e una proprietà id univoca. La creazione di un messaggio e-mail con l’API crea un record basato sul modello ed eventuali metadati aggiuntivi passati. Per una chiamata di creazione e-mail riuscita sono necessari i seguenti parametri: nome, modello, cartella.

I seguenti parametri sono facoltativi per la creazione: `subject`, `fromName`, `fromEmail`, `replyEmail`, `operational`, `isOpenTrackingDisabled`. Se non viene impostato, `subject` sarà vuoto, `fromName`, `fromEmail` e `replyEmail` verrà impostato sui valori predefiniti dell&#39;istanza e `operational` e `isOpenTrackingDisabled` sarà false. `isOpenTrackingDisabled` determina se il pixel di tracciamento aperto viene incluso in un’e-mail quando viene inviato.
...
[Aggiornamento di un’e-mail](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/updateEmailContentUsingPOST) il record può essere eseguito per id. Questo consente di aggiornare la descrizione o il nome dell’e-mail.
...
Il contenuto di ogni sezione di un messaggio e-mail deve essere aggiornato singolarmente, oltre all’oggetto, daName, daEmail e replyEmail, che vengono aggiornati utilizzando [Aggiorna contenuto e-mail](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/updateEmailContentUsingPOST) endpoint. Quando si utilizza questo endpoint, è possibile impostare questi valori anche per l’utilizzo di contenuto dinamico anziché di contenuto statico. Ogni parametro è un oggetto JSON di tipo/valore, dove tipo è &quot;Text&quot; o &quot;DynamicContent&quot; e valore è il valore di testo appropriato o l’ID della segmentazione da utilizzare per il contenuto dinamico. I dati vengono passati come POST x-www-form-urlencoded, non come JSON.  isOpenTrackingDisabled può essere impostato con Update Email Content
...
La maggior parte degli endpoint correlati al modulo richiede un &quot;moduleId&quot; come parametro del percorso. Questa è una stringa che descrive il modulo. moduleIds restituiti da [Ottieni contenuto e-mail](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/getEmailContentByIdUsingGET) come attributo &quot;htmlId&quot; (vedere [Query](#modules_query) sezione successiva).
...
[Recuperare i moduleId e i relativi indici](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/getEmailContentByIdUsingGET) specificando l’id e-mail come parametro del percorso.
...
L’array dei risultati contiene elementi che descrivono sia i moduli che gli elementi HTML. Gli elementi modulo contengono un attributo &quot;contentType&quot;: &quot;Module&quot; e un attributo &quot;index&quot;. Il moduleId viene memorizzato nell’attributo &quot;htmlId&quot;.
...
[Aggiungi un modulo](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/addModuleUsingPOST) per inviare un messaggio e-mail selezionando uno dei moduli esistenti contenuti nel modello e-mail in uso. A tale scopo, specifica l’ID e-mail e il moduleId come parametri del percorso. Il parametro di query dell’indice è obbligatorio e determina l’ordine del modulo nell’e-mail. Se il valore di indice supera il valore di indice esistente più grande, il modulo viene aggiunto all’e-mail.
...
[Ridisponi moduli](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/rearrangeModulesUsingPOST)che contiene tutti i moduli e la posizione desiderata all’interno dell’e-mail per ciascuno di essi. Ogni elemento matrice contiene un oggetto JSON della seguente forma: { &quot;index&quot;: &lt;_index_>, &quot;moduleId&quot;: &quot;&lt;_moduleId_>&quot; }, dove &lt;_index_> è il numero di ordine del modulo basato su zero e &lt;_moduleId_> è l’ID del modulo.
...
[Rinominare un modulo](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/renameUsingPOST) in un messaggio e-mail trasmettendo il nuovo nome tramite il parametro name. Specifica l’ID e-mail e il moduleId (nome esistente) come parametri del percorso.
...
Nell’editor e-mail 1.0, le variabili vengono utilizzate per memorizzare i valori degli elementi nell’e-mail. Ogni variabile viene definita aggiungendo al HTML la sintassi specifica di Marketo come descritto [qui](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/email-marketing/general/email-editor-2/email-template-syntax#EmailTemplateSyntax-Variables). Marketo offre un set di API per la gestione delle variabili all’interno di un messaggio e-mail.

### Query

[Recuperare le variabili](https://developer.adobe.com/marketo-apis/api/asset/#tag/Emails/operation/getEmailVariablesUsingGET) per un’e-mail specificando l’id e-mail come parametro del percorso.
...
Quando si chiama l’endpoint di approvazione, l’e-mail verrà convalidata in base alle regole per le e-mail di Marketo. Il `from name`, `from email`, `reply to email`, e `subject` deve essere compilata prima che l’e-mail possa essere approvata.
...
Il `unapprove` l’operazione può essere eseguita solo su e-mail approvate.
...
Marketo fornisce un metodo semplice per clonare un messaggio e-mail. Questo tipo di richiesta viene effettuata con un application/x-www-url-urlencoded POST e accetta due parametri, nome e cartella obbligatori, un oggetto JSON incorporato con ID e tipo. description è anche un parametro facoltativo. Se non esiste alcuna versione approvata, la versione bozza viene clonata.
...
- type (Tipo): accetta &quot;Text&quot; (Testo) o &quot;HTML&quot; (HTML) per impostazione predefinita.
